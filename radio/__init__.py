"""The ``radio`` module allows devices to communicate using BlueTooth.
...
def send(message: str) -> None:
    """
    Sends a message string -- use this together with ``receive``.

    Currently, this is the equivalent of
    ``send_bytes(bytes(message, 'utf8'))`` but with ``b'\x01\x00\x01'``
    prepended to the front (to make it compatible with other platforms like
    micro:bit).
    """
...
def receive() -> Optional[str]:
    ...
    Returns a received string -- use this together with ``send``.
    ...
    compatible with other platforms like the micro:bit). It strips
...
def config(length: int = 32, queue: int = 3, channel: int = 7,
    ...
    Note, that all parameters need to be passed as keywords
    (i.e. ``config(length=128)`` instead of ``config(128)``.)
...
def send_bytes(message: bytes) -> None:
    """
    Sends a message containing bytes, i.e. raw data.
    """
...
def receive_bytes() -> Optional[bytes]:
    ...
    If a message was sent using ``send``, then the prefix will be returned as
    well -- e.g. after ``radio.send("A")``, then output of
    ``receive_bytes()`` will be ``b'\x01\x00\x01A'``.
...
def receive_full() -> Optional[Tuple[bytes, int, int]]:
    """
    Return the transmitted data as raw bytes message with RSSI (signal
    strength indicator) and us-time-stamp.
    """
