"""
Calliope Mini has a small speaker on-board.
...
def play(music: Union[str, List[str]],
    ...
    """
    Play the music as described by the first argument.
    See the documentation for this module for the Musical Notaton.

    Either pass a single string describing a single note (``music.play("A")``)
    or a list of notes (``music.play(["A", "G4:2"])``).

    Note: You cannot describe multiple notes in a single string!

    You can specify the pin on which the sound signal will get output.
    Default is ``pin30`` which is attached to the built-in beeper.
    You can, for example, use ``pin0`` instead, and attach a speaker to
    ``pin0`` and ``GND``.

    :param wait: Specify whether the command should wait until the music is
    done (True - default) or immediately return and have the music play in the
    "background" (False).
    :param loop: Specify whether to re-start playing the music once it is done
    (defaults to False).
    `
    """
...
def reset() -> None:
    ...
    See the documentation for this module for more info.
